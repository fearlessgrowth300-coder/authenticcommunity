...
import { loadCommunityMessages, sendCommunityMessage, subscribeToCommunityMessages } from '../lib/communityApi'
    ...
    const loadConversations = async () => {
      ...
      try {
        ...
        if (otherUserIds.size > 0) {
          ...
          // Demo fallback conversations for a lively UX
          setConversations([
            {
              userId: 'maya',
              name: 'Maya Patel',
            ...
            },
            {
              ...
              text: 'Thanks for sharing that book recommendation!',
              time: '2d ago',
              unread: 0,
              online: true,
            },
...
/**
 * DirectChat: Full-featured realtime 1-to-1 conversation engine with ui-handoff design shell.
 * Preserves realtime Postgres sync, presence, typing, voice notes, reactions, media, calls, and settings.
 */
export function DirectChat() {
  ...
  const { id } = useParams()
  const { user } = useAuth()
  const { toast: mockToast } = useMockApp()
  ...
  useEffect(() => {
    if (!user || !isPersistedRecipient) {
      if (!isPersistedRecipient) {
        // Seed demo conversation messages for Maya
        setMessages([
          {
            id: 'm1',
            sender_id: 'maya',
  ...
  const handleSend = async (type: string = 'text', stickerUrl?: string) => {
    ...
    if (!isPersistedRecipient) {
      // Local optimistic append for demo contact
...
export function CommunityChat() {
  ...
  const { messages: mockMessages, addMessage, toast: appToast } = useMockApp()
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      loadCommunityMessages(id)
        ...
        .catch(err => appToast(err instanceof Error ? err.message : 'Could not load chat'))
    }
    refresh()
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      appToast('Message sent')
    } catch (err) {
      appToast(err instanceof Error ? err.message : 'Could not send message')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="min-h-[55vh] space-y-4 p-5 bg-white">
            {isPersisted ? (
              ...
              <>
                ...
                />
                {mockMessages.community.map((msg, index) => (
