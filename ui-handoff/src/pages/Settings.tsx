...
export function SettingsHome() {
  ...
  const handleLogout = async () => {
    try {
      ...
      toast(err?.message || 'Error signing out.')
    }
  }
            ...
            <Card key={a as string}>
              <button
                onClick={() =>
                  ...
                  (path.startsWith('/')
                    ? navigate(path)
                    : toast(`${a} settings will be configured soon`))
                }
...
export function AccountSettings() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      if (user) {
        // Mark account as deleted in profiles and sign out
        await supabase
          .from('profiles')
...
export function PrivacySettings() {
  ...
  const toggleLocationPrivacy = async () => {
    ...
    if (user) {
      ...
      toast(nextVal ? 'Exact location hidden (showing city only)' : 'Location visibility updated')
    }
  }
  ...
  const toggleVisibility = async () => {
    ...
    if (user) {
      await supabase
        ...
        .eq('user_id', user.id)
      toast(nextVal ? 'Profile is visible to all members' : 'Profile is visible to connections only')
    }
  }
...
export function NotificationSettings() {
  const { settings, toggleSetting } = useMockApp()
    ...
    <AppShell title="Notification Settings" subtitle="Choose which updates deserve your attention">
      <div className="mx-auto max-w-2xl space-y-5">
        <Card className="p-5">
          ...
          <div className="mt-4 divide-y divide-brand-line">
            <Toggle id="messages" title="Messages" detail="New messages and replies" />
            <Toggle id="connections" title="Connection requests" detail="People who want to connect" />
            <Toggle id="communities" title="Communities" detail="Posts, comments and mentions" />
            <Toggle id="events" title="Events" detail="Event updates and reminders" />
            <Toggle id="digest" title="Weekly digest" detail="Your weekly community summary" />
          </div>
        </Card>
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 divide-y divide-brand-line">
            <Toggle id="push" title="Push notifications" detail="Instant alerts on this device" />
            <Toggle id="email" title="Email notifications" detail="Receive selected updates by email" />
          </div>
        </Card>
      </div>
    </AppShell>
...
function Toggle({ id, title, detail }: { id: string; title: string; detail: string }) {
  const { settings, toggleSetting } = useMockApp()
