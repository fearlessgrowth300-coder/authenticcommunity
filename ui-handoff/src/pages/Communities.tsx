...
import { joinCommunity as joinPersistedCommunity, loadCommunities, loadCommunity, createCommunity, type CommunityRecord } from '../lib/communityApi'
...
export function Communities() {
  ...
  const source = remote.length ? remote : communities
...
export function CommunityDetail() {
  const nav = useNavigate()
  const { id } = useParams()
  const { joinedCommunityIds, toggleCommunity, toast } = useMockApp()
  ...
  const c = remoteDetail || communities.find(x => x.id === id) || communities[0]
  const e = events[0]
  ...
  const join = async () => {
    if (remoteDetail) {
      try {
        ...
        toast(result === 'joined' ? "You're in!" : 'Request sent to the community admins')
      } catch (error) {
      ...
      <div className="mx-auto max-w-4xl space-y-5">
          ...
          <div className="p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex gap-2">
                <Button onClick={join}>
                  {joined ? 'Joined ✓' : remoteDetail?.approvalRequired ? 'Request to join' : 'Join Community'}
                </Button>
        ...
        <Card className="p-5">
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            <img src={e.image} className="h-36 w-full rounded-2xl object-cover sm:w-48" alt={e.title} />
            <div className="flex-1">
              <div className="text-lg font-extrabold">{e.title}</div>
              <div className="mt-1 text-sm text-brand-muted">
                {e.date} · {e.time} · {e.distance}
              </div>
              <p className="mt-3 text-sm text-slate-600">
                Start your weekend with intention and meet people from the community in person.
              </p>
              <Button variant="secondary" className="mt-4" onClick={() => nav(`/events/${e.id}`)}>
                View event
              </Button>
            </div>
          </div>
...
export function CreateCommunity() {
  ...
  const handleCreate = async () => {
    if (!name.trim()) {
      toast('Please enter a community name.')
      return
    }
    ...
    try {
      ...
      toast(`Community "${name}" created successfully!`)
      nav(`/communities/${newId}`)
    } catch (err: any) {
      toast(err?.message || 'Failed to create community.')
    } finally {
      setSubmitting(false)
    }
  }
