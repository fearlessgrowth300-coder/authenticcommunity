...
export async function joinCommunity(community: CommunityRecord): Promise<'joined' | 'requested'> {
  ...
  if (community.approvalRequired) {
    const { error } = await (supabase as any)
      ...
      .upsert(
        { community_id: community.id, user_id: auth.user.id, status: 'pending' },
  ...
  const { error } = await (supabase as any)
    ...
    .upsert(
      { community_id: community.id, user_id: auth.user.id, role: 'member', status: 'active' },
      { onConflict: 'community_id,user_id' }
    )
  if (error) throw error
  return 'joined'
    ...
    .insert({
      ...
      community_type:
        params.privacy === 'Private'
          ? 'private'
          : params.privacy === 'Approval required'
          ? 'approval_required'
          : 'public',
...
const fallbackEventImage =
  'https://images.unsplash.com/photo-1551632811-561732d1e306?auto=format&fit=crop&w=1100&q=85'
...
export async function rsvpToEvent(eventId: string, currentRsvpState: boolean): Promise<boolean> {
  ...
  if (currentRsvpState) {
    ...
    if (evt && evt.attendee_count > 0) {
      await (supabase as any)
        .from('events')
        .update({ attendee_count: evt.attendee_count - 1 })
        .eq('id', eventId)
    }
    return false
  } else {
    ...
    await (supabase as any)
      .from('events')
      .update({ attendee_count: (evt?.attendee_count ?? 0) + 1 })
      .eq('id', eventId)
    return true
  }
}
