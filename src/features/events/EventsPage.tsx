...
import { Button, Card, Chip, Field, inputClass } from '@/components/ui/AppUi'
import { toast } from 'sonner'
import { loadEvents, loadEventDetail, rsvpToEvent, createEvent, type EventRecord } from '@/features/communities/communityApi'

const defaultEvents: EventRecord[] = [
  {
    id: 'e1',
    title: 'Saturday Morning Trail Walk & Coffee',
    host: 'Austin Hikers',
    date: 'Aug 22',
    time: '8:30 AM',
    distance: 'Barton Creek Greenbelt',
    attendees: 18,
    category: 'Outdoors',
    description: 'An easy 3-mile walk followed by casual coffee at a local cafe. All paces and first-timers welcome!',
    image: 'https://images.unsplash.com/photo-1551632811-561732d1e306?auto=format&fit=crop&w=1100&q=85',
  },
  {
    id: 'e2',
    title: 'Founder Coffee & Candid Chats',
    host: 'Startup Circle',
    date: 'Aug 24',
    time: '7:00 PM',
    distance: 'Downtown Coffee Lab',
    attendees: 12,
    category: 'Business',
    description: 'Informal gathering for builders, creators and curious minds. No pitch decks, just honest conversations.',
    image: 'https://images.unsplash.com/photo-1511632765486-a01980e01a18?auto=format&fit=crop&w=1100&q=85',
  },
]
...
export function Events() {
  ...
  const source = remoteEvents.length > 0 ? remoteEvents : defaultEvents
...
export function EventDetail() {
  ...
  const e = remoteEvent || defaultEvents.find(x => x.id === id) || defaultEvents[0]
  ...
  const handleRsvp = async () => {
    ...
    try {
      if (remoteEvent) {
        ...
        toast.success(nextState ? "You're going! ✓" : 'RSVP cancelled')
      } else {
        ...
        <Card className="overflow-hidden">
          ...
          <div className="p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <Button variant="secondary" onClick={() => toast.success(`You are following ${e.host}`)}>
                Follow host
              </Button>
            </div>
      ...
      <div className="mx-auto max-w-2xl">
        <Card className="p-6">
          <Field label="Cover photo">
            <button
              type="button"
              onClick={() => toast.info('Default high-resolution cover assigned')}
