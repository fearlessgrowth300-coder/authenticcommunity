...
import { Button, Card, Chip, Verified } from '@/components/ui/AppUi'
...
import { scoreCandidateMatch } from '@/lib/matching'
...
export function Matches() {
  ...
  useEffect(() => {
    ...
    const loadCandidates = async () => {
      ...
      try {
        ...
        toast.error(err?.message || 'Error loading discover matches.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      toast.info(err?.message || 'Connection request submitted.')
      navigate(`/matches/${p.id}`)
    }
  }
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      if (followStatus === 'following' || followStatus === 'requested') {
        ...
        toast.success(nextStatus === 'requested' ? 'Follow request sent' : 'Now following member')
      }
    } catch (err: any) {
  ...
  const handleConnectAction = async () => {
    ...
    try {
      if (connectionStatus === 'none') {
        ...
        toast.success(`Connection request sent to ${profile?.first_name || 'member'}`)
      } else if (connectionStatus === 'pending_incoming') {
        await acceptConnectionRequest(targetUserId)
        setConnectionStatus('connected')
        toast.success(`You are now connected with ${profile?.first_name || 'member'}!`)
        ...
        navigate(`/messages/direct/${targetUserId}`)
      }
    } catch (err: any) {
      toast.error(err?.message || 'Action failed.')
    } finally {
          ...
          <>
            ...
            <div className="sticky bottom-24 w-full lg:bottom-4 flex gap-3">
              {connectionStatus === 'pending_incoming' ? (
                ...
                <Button
                  className="w-full py-3"
                  onClick={() => navigate(`/messages/direct/${targetUserId}`)}
                >
