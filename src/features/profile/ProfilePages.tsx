...
import { Avatar, Button, Card, Chip, Field, inputClass, SectionHeader, Verified } from '@/components/ui/AppUi'
...
export function Followers() {
  ...
  const handleToggleFollow = async (member: SocialMember) => {
    try {
      if (member.isFollowing) {
        ...
        toast.success(res === 'requested' ? `Follow request sent to ${member.name}` : `Following ${member.name}`)
      }
    } catch (err: any) {
  ...
  const handleRemoveFollower = async (member: SocialMember) => {
    try {
      ...
      toast.success(`Removed ${member.name} from followers`)
    } catch (err: any) {
  ...
  const handleAcceptFollowReq = async (req: SocialMember) => {
    try {
      ...
      toast.success(`Accepted follow request from ${req.name}`)
    } catch (err: any) {
  ...
  const handleRejectFollowReq = async (req: SocialMember) => {
    try {
      ...
      setPendingRequests(curr => curr.filter(r => r.userId !== req.userId))
      toast.success(`Declined follow request from ${req.name}`)
    } catch (err: any) {
                ...
                <div className="flex items-center gap-2">
                  {tab === 'Followers' ? (
                    ...
                    <Button
                      variant="secondary"
                      className="text-xs py-1.5 px-3"
                      onClick={() => navigate(`/messages/direct/${member.userId}`)}
                    >
...
export function Connections() {
  ...
  const handleRemove = async (conn: SocialMember) => {
    ...
    try {
      ...
      toast.success(`Removed connection with ${conn.name}`)
    } catch (err: any) {
              ...
              <Card key={p.userId} className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="secondary"
                    className="text-xs py-1.5 px-3"
                    onClick={() => navigate(`/messages/direct/${p.userId}`)}
                  >
                    <MessageCircle className="h-3.5 w-3.5" /> Message
                  </Button>
                  <button
