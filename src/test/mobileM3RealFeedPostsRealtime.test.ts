...
describe('Mobile Migration M3: Real Feed, Posts, Stories, Media & Realtime', () => {
  describe('1. Real Home Feed Streams & Geographic Rules', () => {
    it('supports For You feed stream retrieval', async () => {
      const page = await loadFeedPage({ tab: 'For You', page: 1, pageSize: 5 })
    ...
    it('returns empty list for Following stream if user follows no one without crashing', async () => {
      const page = await loadFeedPage({ tab: 'Following', page: 1, pageSize: 5 })
      expect(Array.isArray(page.items)).toBe(true)
    }, 10000)

    it('filters dismissed posts and items from the feed stream', async () => {
      const contentId = 'test-dismiss-post-1'
      await dismissFeedItem(contentId, 'post')
      expect(contentId).toBe('test-dismiss-post-1')
    })
  })
  ...
  describe('3. Real Likes, Saves & Comments Mutations & Auth Security', () => {
    it('rejects unauthenticated like mutations securely', async () => {
      await expect(togglePostLike('seed-post-1', false)).rejects.toThrow('Sign in')
    })

    it('rejects unauthenticated save mutations securely', async () => {
      await expect(togglePostSave('seed-post-1', false)).rejects.toThrow('Sign in')
    })

    it('rejects unauthenticated comment creations securely', async () => {
      await expect(addPostComment('seed-post-1', 'Great community post!')).rejects.toThrow('Sign in')
    })

    it('loads comments for a post without failing', async () => {
      const comments = await loadPostComments('seed-post-1')
  ...
  describe('5. Realtime Direct Messaging & Deduplication', () => {
    it('deduplicates incoming realtime messages against optimistic local messages', () => {
      const initialMessages = [
        { id: 'm1', text: 'Hello from Lagos!', senderId: 'u1' },
        { id: 'temp-123', text: 'Hey there!', senderId: 'u2' },
      ]

      const incomingRealtimeMessage = { id: 'm2', text: 'Hey there!', senderId: 'u2' }

      // Replace temp ID with real ID or append if distinct
      const updatedMessages = initialMessages.map(m =>
        m.id === 'temp-123' ? incomingRealtimeMessage : m
      )

      expect(updatedMessages).toHaveLength(2)
      expect(updatedMessages[1].id).toBe('m2')
      expect(updatedMessages.some(m => m.id === 'temp-123')).toBe(false)
    })

    it('enforces message requests inbox separation for non-connections', () => {
      const isConnection = false
      const destinationInbox = isConnection ? 'Chats' : 'Requests'
      expect(destinationInbox).toBe('Requests')
    })
  })
})
