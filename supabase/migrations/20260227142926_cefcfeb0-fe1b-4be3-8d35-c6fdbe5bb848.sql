-- Recreate the set_admin_access_pin function using extensions schema for crypt/gen_salt
...
BEGIN
  ...
  hashed := extensions.crypt(trim(_pin), extensions.gen_salt('bf'));
...
BEGIN
  ...
  RETURN stored_hash = extensions.crypt(trim(_pin), stored_hash);
END;
$function$;